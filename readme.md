...
- Sentence fixed expressions are checked by default and synced as phrase records to the vocabulary library; unchecking removes only records generated by that sentence.
- Detect and highlight discontinuous constructions such as `not only … but also`.
- Hide or reveal translations in the side panel and exported HTML for recall practice.
