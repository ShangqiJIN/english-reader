...
export function createHtml(items, title = "English Reader 学习库") {
  ...
  const cards = items.map((item) => {
    ...
    const detail = item.meanings?.length
      ? `<ol class="meanings">${item.meanings.map((meaning) => `<li><strong>${escapeHtml(partName(meaning.partOfSpeech))}</strong> ${escapeHtml(meaning.definitionZh ?? meaning.definitionEn ?? "")}</li>`).join("")}</ol>`
      : `<p>${escapeHtml(item.kind === "sentence" ? item.translationZh ?? "" : item.chineseDefinition ?? "")}</p>`;
    const ipa = item.ipa ? `<p class="ipa">${escapeHtml(item.ipa)}</p>` : "";
    const collocations = item.collocations?.length ? `<div class="collocations"><strong>固定搭配</strong><ul>${item.collocations.map(({ phrase, meaningZh }) => `<li><strong>${escapeHtml(phrase)}</strong> — ${escapeHtml(meaningZh)}</li>`).join("")}</ul></div>` : "";
    const date = item.createdAt ? new Date(item.createdAt).toLocaleString("zh-CN") : "";
    const meta = [language, item.source?.pageTitle, date].filter(Boolean).map(escapeHtml).join(" · ");
    return `<article class="item"><h2>${escapeHtml(item.text ?? "")}</h2>${detail}${ipa}${collocations}<p class="meta">${meta}</p></article>`;
  }).join("\n");
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><style>:root{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}*{box-sizing:border-box}body{max-width:860px;margin:0 auto;padding:24px;background:#f5f1e8;color:#25231e}header{margin:-24px -24px 20px;padding:24px;background:#173f35;color:white}header p{margin:6px 0 0;color:#d8e6df}.item{margin:10px 0;padding:16px;border:1px solid #ddd5c6;border-radius:12px;background:#fffdf8}.item h2{margin:0 0 8px;font-size:18px}.item p{color:#625d53}.ipa{color:#355f50;font-family:Georgia,serif}.meanings{padding-left:24px}.collocations{margin-top:12px;color:#355f50}.collocations ul{margin:5px 0;padding-left:22px}.meta{color:#938b7d!important;font-size:11px}</style></head><body><header><h1>${escapeHtml(title)}</h1><p>${items.length} 条当前筛选记录</p></header><main>${cards || "<p>没有学习记录。</p>"}</main></body></html>`;
}
